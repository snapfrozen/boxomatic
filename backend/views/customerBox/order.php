		...
		<div class="section-container auto" data-section>
								...
								<li class="<?php echo $classes; ?>">
									<div class="row">
										<div class="large-4 columns">
											<p><?php echo SnapFormat::dayOfYear($DeliveryDate->date) ?></p>
											<?php 
												$Order=Order::model()->findByAttributes(array('delivery_date_id'=>$DeliveryDate->id, 'user_id'=>Yii::app()->user->user_id));
												if(!$Order) {
													$Order=new Order;
													$Order->delivery_date_id=$DeliveryDate->id;
													$Order->user_id=Yii::app()->user->user_id;
													$Order->location_id=$Customer->Location->location_id;
													$Order->save();
												}
												echo CHtml::hiddenField('delivery_price_'.$Order->id, $Order->Location->location_delivery_value);
											?>
										</div>
										...
										<div class="large-8 columns">
											<?php
												...
												echo CHtml::dropDownList('CustDeliveryDates[' . $Order->id . ']', $Order->location_key, $Order->Customer->getDeliveryLocations(),$attribs); 
											?>
										</div>
									</div>
									...
									<div class="row">
										...
										<div class="large-6 columns">
											<table>
												...
												<tbody>
													<tr>
														<td class='boxes'><?php echo SnapFormat::currency($Customer->totalBoxesByDeliveryDate($DeliveryDate->id)) ?></td>
														<td class='extras'>?</td>
														<td class='delivery'><?php echo SnapFormat::currency($Customer->totalDeliveryByDeliveryDate($DeliveryDate->id)); ?></td>
														<td class='total'>
															<strong><?php echo SnapFormat::currency($Customer->totalByDeliveryDate($DeliveryDate->id)) ?></strong>
															<?php if(Yii::app()->user->shadow_id): ?>
															<?php echo CHtml::link('update',array('deliveryDate/updateUserBoxes','date_id'=>$DeliveryDate->id,'cust_id'=>$Customer->user_id), array('class' => 'button small')); ?>
															<?php endif; ?>
														</td>
													</tr>
												</tbody>
										...
										<div class="large-12 columns">
											<?php echo CHtml::link('Update Extras',array('OrderItem/order','date'=>$DeliveryDate->id),array('class'=>'button small')) ?>
										</div>
			...
			<section>
				...
				<div class="content" data-section-content>
					...
					<?php echo CHtml::link('Add new delivery location',array('customerLocation/create','custId'=>$Customer->user_id,'order'=>1), array('class' => 'button small'))?>
	...
	<div class="large-4 columns">
		...
		<?php $this->widget('zii.widgets.CListView', array(
			'dataProvider'=>new CActiveDataProvider('UserPayment',array(
				'criteria'=>array(
					'condition'=>'user_id='.$Customer->user_id,
