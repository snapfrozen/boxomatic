 ...
 */
class BoxomaticUserIdentity extends CUserIdentity
{
	private $_id;
	...
	*/
	...
	{		
		$user=User::model()->resetScope()->findByAttributes(array('email'=>$this->username));
		if($user===null)
		{
	...
	{	
		if(!Yii::app()->user->shadow_id) {
			...
			$this->setState('shadow_name', $shadow->email);
